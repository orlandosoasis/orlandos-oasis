...
import { Bell, CheckCheck, Wrench, UserCheck, UserX, CheckCircle2, AlertTriangle, X } from "lucide-react";
...
function NotifIcon({ kind }: { kind: string }) {
  ...
  if (kind.startsWith("route_")) return <AlertTriangle className={cn(base, "text-amber-500")} />;
        ...
        <div className="space-y-2">
          {notifs.map((n) => (
            <div
              ...
              className={cn(
                "flex items-start gap-3 rounded-2xl border border-border p-4 transition-colors",
                n.cta_route ? "cursor-pointer hover:bg-muted/50" : "",
